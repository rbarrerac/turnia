...
| 18/09/2026 | Luis | incidente | **QA — dos pruebas unitarias no deterministas, dependientes del reloj real.** `gestorDeReservas.prueba.js` ("cancelar con menos de 2 horas de anticipación lanza CANCELACION_TARDIA") y `gestorDeNotificaciones.prueba.js` ("enviarRecordatorio registra una notificación de tipo recordatorio") eran las únicas dos pruebas de esas suites que anclaban la cita de prueba a `Date.now()` (offsets de 30 y 120 minutos desde "ahora"), en vez de a un día/hora fija como el resto (`diaLaborable`, "lunes de la próxima semana", que sí es una función determinista de la fecha de ejecución). Eso las dejaba expuestas a dos problemas de fondo: (1) `GestorDeReservas.cancelar` compara `cita.inicia_en` contra `Date.now()` real, así que la prueba de RN-03 dependía de que ambos instantes ("ahora" al crear la cita y "ahora" al cancelar) coincidieran con la ejecución real, sin ningún punto de control; (2) el helper `crearCitaDePrueba`/`crearCita`, al anclar a una ventana estrecha "ahora + N minutos" en vez de a un hueco de horario fijo y exclusivo, competía por el mismo rango de tiempo que otras suites o corridas previas contra la base de datos compartida de desarrollo (Jest corre los archivos de prueba en procesos paralelos), pudiendo agotar sus reintentos ante la restricción `sin_superposicion` y fallar con "No se pudo crear la cita de prueba tras varios reintentos" según el momento exacto de ejecución — un resultado no reproducible entre máquinas o corridas. |
| 18/09/2026 | Luis | decisión | **Solución — reloj inyectable (Opción A) + instantes fijos, sin tocar reglas de negocio.** `GestorDeReservas` ahora recibe un `reloj` opcional en el constructor (`constructor(reloj = () => new Date())`), usado en los tres lugares donde antes leía la hora directamente (`Date.now()`/`new Date()` en `calcularDisponibilidad`, `cancelar` y `cambiarEstado`); por defecto sigue siendo el reloj real, así que el comportamiento de producción (RN-02, RN-03, RN-06, RN-08) queda idéntico — solo las pruebas lo sobreescriben. No se tocó `GestorDeNotificaciones` ni `recordatorios.tarea.js`: `enviarRecordatorio`/`enviarConfirmacion` no validan ninguna regla de horario, así que la prueba señalada no necesitaba un reloj inyectado en esa clase, solo un punto de partida fijo para crear su cita (ver abajo) — se prefirió no añadir una abstracción que ninguna prueba iba a usar. En `gestorDeReservas.prueba.js`: se agregó `AHORA_CONGELADA` (20:00 del mismo `diaLaborable` ya usado por el archivo, fuera de los rangos 9:00–10:30 que ya ocupan las pruebas de "reservar") y un `gestorConRelojCongelado = new GestorDeReservas(() => new Date(AHORA_CONGELADA))`, usado solo por la prueba de RN-03; `crearCitaDePrueba` ganó un parámetro opcional `instanteBase` (por defecto `new Date()`, sin cambiar el comportamiento de las demás pruebas que no lo pasan). En `gestorDeNotificaciones.prueba.js`: se agregó `INSTANTE_FIJO_RECORDATORIO` (45 días en el futuro, calculado una sola vez al cargar el archivo, lejos de cualquier otra cita de esta u otras suites) y el mismo parámetro `instanteBase` en `crearCita`, usado solo por la prueba de `enviarRecordatorio`. La prueba de `enviarRecordatoriosPendientes` (RN-06) se dejó intacta (sigue usando `Date.now()`): su corrección depende de que el instante de las citas de prueba y el `new Date()` real que usa `recordatorios.tarea.js` coincidan, y no era una de las dos pruebas señaladas como no deterministas ni mostró fallos. Aislamiento: ambas pruebas modificadas siguen registrando el id de la cita creada en `idsDeCitasCreadas`, limpiado en `afterAll` como ya hacía el resto del archivo. |
| 18/09/2026 | Luis | avance | **Verificación.** `npm run prueba`: **65 pruebas pasan, 0 `todo`**, en 6 suites (sin cambios respecto al conteo anterior; las 63 pruebas restantes no se tocaron). Ejecutado dos veces seguidas con el reloj real, ambas en verde y sin datos huérfanos. Independencia del reloj verificada aparte, congelando `Date`/`Date.now()` globalmente con un script de carga temporal (`--require`, fuera del repositorio) a dos instantes que antes exponían el problema: domingo 20/09/2026 10:00 UTC (día no laborable) y viernes 18/09/2026 17:30 UTC (cerca del cierre de las 18:00) — en ambos casos las 65 pruebas, incluidas las dos señaladas, pasaron sin cambios. |
