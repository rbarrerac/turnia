...
<<<<<<< Updated upstream
=======
| 15/09/2026 | Ronald | avance | Incremento 1 (Autenticación, CU-01/CU-02) — Backend: `GestorDeAutenticacion.registrar/iniciarSesion/obtenerPerfil` con bcrypt (10 rondas), validación de correo/contraseña, unicidad de correo (RN-07); `generadorDeTokens` (JWT) y `validadores` implementados; middleware `verificarAutenticacion` (lee `Authorization: Bearer`, adjunta `req.usuario = { id, rol }`) y `exigirRol(...roles)` (antes `verificarRol`, renombrado y propagado a las rutas de servicios/citas/horarios/usuarios); controlador y rutas de `/api/autenticacion` conectados a la lógica real; `manejadorErrores` ajustado para no registrar en consola errores esperados (4xx). |
| 15/09/2026 | Luis | avance | Incremento 1 (Autenticación) — Frontend: `ContextoAutenticacion` funcional (usuario, token, `iniciarSesion`, `registrar`, `cerrarSesion`, persistencia en `localStorage`); `RutaProtegida` redirige a `/inicio-sesion` sin sesión o a `/` si el rol no coincide; `PaginaRegistro` y `PaginaInicioSesion` con formularios, validación en cliente y mensajes de error/éxito en español; `PaginaInicioSesion` redirige según rol (`administradora` → `/admin/agenda`, `clienta` → `/servicios`); `BarraNavegacion` muestra estado de sesión; `App` envuelta con el proveedor de autenticación en `main.jsx`. |
| 15/09/2026 | Luis | avance | Incremento 1 (Autenticación) — Pruebas: `pruebas/integracion/autenticacion.prueba.js` con Jest + Supertest cubre registro (201), correo duplicado (409), contraseña corta (400), inicio de sesión correcto (200 + token) e incorrecto (401), y acceso a `/perfil` sin token (401) y con token válido (200). Las 7 pruebas pasan en verde. |
| 15/09/2026 | Ronald | decisión | Se creó `api/src/aplicacion.js` separando la construcción de la app Express (usada por las pruebas con Supertest) de `api/src/servidor.js` (que solo llama a `.listen()`), para poder probar la API sin abrir un puerto real. |
| 15/09/2026 | Ronald | decisión | Las pruebas de integración usan una base de datos PostgreSQL de prueba dedicada (`turnia_prueba`), separada de la de desarrollo (`turnia`), configurada vía `api/.env.prueba` (no versionado; plantilla en `api/.env.prueba.example`) cargado por `pruebas/configuracionPruebas.js` como `setupFiles` de Jest. Los datos creados por las pruebas se limpian en `beforeAll`/`afterAll` para que la corrida sea repetible. |
>>>>>>> Stashed changes
